...
created: 2026-05-15
updated: 2026-06-01
summary: 完整剖析 CLAUDE.md 的文件层级、加载机制、rules 路径作用域、渐进式披露指令与最佳实践。
confidence: high
---
...
1. **`@import` 语法（推荐）**——在根 CLAUDE.md 中直接引用，启动时立即展开。支持相对/绝对路径，递归最多 4 层（four hops，官方上限）。代价是所有内容进入上下文消耗 token。
...
2. **`--add-dir` + 环境变量**：`--add-dir` 用于把**主工作目录之外**的目录纳入 Claude 可访问范围（如同级共享配置仓、monorepo 的兄弟包），配合环境变量即可一并加载这些外部目录的 CLAUDE.md：

   ```bash
   CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1 claude --add-dir ../shared-config
   ```

   （注意：`--add-dir` 面向工作目录**之外**的路径；工作目录内的子目录用方法 1 的 `@import` 或方法 3 的主动触发即可。）
...
CLAUDE.md 的写作规范、两种机制的选择决策树、推荐完整目录结构、调试方法，以及与 Auto Memory 的互补关系。

详见 [[topics/claude-md-best-practices]]。
...
- [[topics/claude-md-best-practices]]
